...
void loop()
{
  ...
  //PID
  int pid_torque = 15*theta+80*(theta-prev_theta)+ 20*x+0*(x-prev_x); //50 450 //20-20 good
 ...
 //PREVIOUS STATE VARIABLES
  ...
  //Serial.print(pid_torque); Serial.print("\t");
  //Serial.println(constrain(pid_torque,-100,100));
  
  if(n!=1)
  {
    omega = ((lqr_torque + prev_lqr_torque)*dt2)/I_b; 
    //Serial.println(omega);
  }
  //torque = constrain(omega, -220, 220);
